---
name: python-expert
description: Expert-level Python author for non-trivial implementation work in the installkernel-wsl project. Use when the task needs idiomatic, production-quality Python beyond a simple edit.
---

# Python Expert Agent

General-purpose expert-level Python coding agent for the installkernel-wsl project.
...
- `` :py:mod:`installkernel_wsl.string` `` for modules
- `` :py:func:`installkernel_wsl.string.slugify` `` for functions
- `` :py:class:`installkernel_wsl.typing.ProbeDict` `` for classes
- `` :py:meth:`MyClass.my_method` `` for methods
- Use `~` to shorten the displayed name:
  `` :py:func:`~installkernel_wsl.string.slugify` `` renders as `slugify`
