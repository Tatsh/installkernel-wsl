...
Fixes mypy type errors and eliminates `Any` usage in the installkernel-wsl project.
...
1. Fix all mypy errors reported by `uv run mypy installkernel_wsl`
...
Place new TypedDict classes in `installkernel_wsl/typing.py` if they are used across
modules, or locally if single-use. Add them to `__all__` in
`installkernel_wsl/typing.py` when shared.
...
1. Run `uv run mypy installkernel_wsl` and capture all errors.
...
4. Run `uv run mypy installkernel_wsl` again to verify fixes.
