...
If any changed files are under `installkernel_wsl/` or `tests/`, run the following
agents **in order**:
...
1. **click-auditor** - validate Click command consistency. **Only run if files under
   `installkernel_wsl/commands/` changed.**
...
- **changelog** - update `CHANGELOG.md` only for **user-facing** changes (behaviour, CLI, public
  ...
  Files under `installkernel_wsl/`, `tests/`, or version changes in `pyproject.toml` are **candidates**
...
For example, if a commit contains `installkernel_wsl/commands/main.py`,
...
For Python files, strip the `installkernel_wsl/` prefix and replace `/` with `.` (like module imports).

- Python file `installkernel_wsl/media.py` → `media:`.
- Multiple files under `installkernel_wsl/commands/` → `commands:`.
- Single command file `installkernel_wsl/commands/admin.py` → `commands.admin:`.
